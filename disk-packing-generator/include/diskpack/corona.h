...
class Corona {
  ...
  std::deque<const Disk *> corona;                        /// Peripheral disks. At any moment two consequitive disks are tangent.
                                                          /// (i. e. disks with indexes i and i + 1 where i + 1 < corona.size())
  ...
  SpiralSimilarityOperator GetOperatorsProduct(           /// Helper function for compuitng new disks' centers
      const size_t &begin, const size_t &end,
      const std::vector<SSORef> &operators) const;

  void GetSortedCorona(const std::list<Disk> &packing);   /// Collects disks tangent to the base and sorts them anti-clockwise
