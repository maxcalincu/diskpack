...
class PackingGenerator {
  ...
  const BaseType
      packing_radius; /// Determines the size of the generated packing
  const BaseType
      precision_upper_bound; /// Upper bound for disks' interval width
  std::list<Disk> packing;   /// Current state of the packing
  QueueType
      disk_queue; /// Queue of disks managed by PackingGenerator::Advance()
  ...
  PackingStatus
  GapFill(Corona &corona); /// Recursive function which adds disks to the
                           /// packing while corona is not complete
  PackingStatus
  AdvancePacking(); /// Recursive function which picks a disk from the queue and
                    /// fills it's corona using GapFill

  void Push(Disk &&new_disk,
            size_t index); /// Pushes the disk into the packing and updates the
                           /// state of the class
  ...
  inline bool HasIntersection(const Disk &new_disk)
      const; /// Determines whether new_disk (certainly) intersects any other
...
public:
  ...
  GetGeneratedRadius(); /// Returns the radius of the generated packing
                        ...
                        /// FindPacking returns anything other than
                        /// PackingStatus::complete)
  const BaseType &
  GetRadius(); /// Dumps the packing to a storage once it's completed
  const std::list<Disk> &
  GetPacking(); /// Getter function for the generated packing
