...
bool Corona::PeekNewDisk(Disk &new_disk, size_t index, const std::optional<ConnectivityGraph> &graph) {
  bool use_front = corona.front()->precision() < corona.back()->precision();

  if (graph.has_value()) {
    if (!graph->HasTriangle(base.get_type(), use_front ? corona.front()->get_type() : corona.back()->get_type(), index)) {
      return false;
    }
  }
...
ConnectivityGraph::ConnectivityGraph(OperatorLookupTable &lookup_table):  diffs(lookup_table.radii.size()), 
                                                                          signatures(lookup_table.radii.size()),
                                                                          transitions(  lookup_table.radii.size(), 
                                                                                    std::vector<size_t>((  lookup_table.radii.size() * 
                                                                                                                    lookup_table.radii.size() + 
                                                                                                                    lookup_table.radii.size())/2, 0)),
  ...
  for (size_t base = 0; base < radii.size(); ++base) {
    ...
    for (size_t starting_index = 0; starting_index < radii.size(); ++starting_index) {
      packing.push_back(std::make_shared<Disk>(radii[base] + radii[starting_index], zero, radii[starting_index], starting_index));
...
void ConnectivityGraph::Refine(OperatorLookupTable &lookup_table) {
  ...
  std::shared_ptr<std::vector<std::shared_ptr<SignatureList>>> invalid_signatures = std::make_shared<std::vector<std::shared_ptr<SignatureList>>>(diffs.size());
...
bool ConnectivityGraph::Restore() {
  if (std::any_of(diffs.begin(), diffs.end(), [](const VersionDiffStack &s) { return s.empty(); })) {
    return false;
  }
...
void ConnectivityGraph::RemoveRedundantTriangles(std::shared_ptr<std::vector<std::shared_ptr<SignatureList>>> diff) {
  ...
  if (diff) {
    for (size_t base = 0; base < edges.size(); ++base) {
      diff->operator[](base)->splice(diff->operator[](base)->end(), std::move(removed_signatures[base]));
    }
  }
}
  ...
  for (size_t i = 0; i < edges.size(); ++i) {
    for (size_t j = i; j < edges.size(); ++j) {
      ...
      for (size_t x = 0; x < edges.size(); ++x) {
        edges[i][j] = edges[i][j] || (GetTransitionsConst(i, j, x) > 0 && GetTransitionsConst(j, i, x) > 0);
        edges[j][i] = edges[i][j];
      }
    }
  }
...
bool ConnectivityGraph::IsViable() const {
  //graph G with one vertex for each disk size, and an edge between i and j (j can be equal to i, i.e. they may have loop) if there is a disk x s.t.
  // * there is an i-corona with an angle jx (that is, two consecutive disks of radius j and x)
  ...
  // DisplaySignatures();
  {
    ...
    if (std::any_of(unvisited.begin(), unvisited.end(), [](bool b) {return b;})) {
      return false;
    }
  ...
  {
    for (size_t disconnected_vertice = 0; disconnected_vertice < edges.size(); ++disconnected_vertice) {
      ...
      if (!std::any_of(signatures[disconnected_vertice].begin(), signatures[disconnected_vertice].end(), 
          ...
          for (size_t component = 1; component <= cur_component; ++component) {
            ...
            for (size_t vertice = 0; vertice < edges.size(); ++vertice) {
              connects_component = connects_component || (signature->disks[vertice] && components[vertice] == component);
            }
  ...
  // Condition 3: the graph H whose vertices are the coronas with an edge between two coronas if they are "compatible" 
  //              (which means there is a triangle i,j,x for some x in the previous graph G). 
  //              This graph H must contains a connected component with all types of coronas (i. e. with all types of corona bases).
  {
    ...
    for (size_t base = 0; base < edges.size(); ++base) {
      for (auto signature : signatures[base]) {
        size_t first_i = signature->specimen_indexes[0], first_j = signature->specimen_indexes[1];
    ...
    std::vector<std::vector<bool>> component_disk_types(edges.size() * edges.size() * edges.size(), std::vector<bool>(edges.size(), false));
    for (size_t i = 0; i < edges.size(); ++i) {
      for (size_t j = i; j < edges.size(); ++j) {
...
bool ConnectivityGraph::HasTriangle(size_t i, size_t j, size_t k) const {
  return GetTransitionsConst(i, j, k) && GetTransitionsConst(k, i, j) && GetTransitionsConst(j, k, i);
}
