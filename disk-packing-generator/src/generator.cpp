...
BasicGenerator::BasicGenerator(const std::vector<Interval> &radii_,
    ...
    : radii(radii_), packing_radius{packing_radius_},
      ...
      max_ignored_radii(max_ignored_radii_), disk_queue(LessNormCompare),
      frequency_table(radii_.size(), 0), lookup_table(radii_),
      generated_radius(0), g(rd()), graph(lookup_table) {
...
PackingStatus BasicGenerator::Generate(const size_t &central_disk_type) {
  ...
  Reset();
  Push(Disk(zero, zero, radii[central_disk_type], central_disk_type),
       central_disk_type);
...
bool BasicGenerator::HasIntersection(const Disk &new_disk) const {
  return std::any_of(packing.begin(), packing.end(),
...
bool BasicGenerator::PackingSatisfiesConstraints() const {
  return std::count(frequency_table.begin(), frequency_table.end(), 0) <=
         max_ignored_radii;
}
