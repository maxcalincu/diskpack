...
namespace CDP {
/// RadiiRegion
    void RadiiRegion::Split(std::vector<RadiiRegion> &regions, int k) const {
        k = std::max(k, 1);               /// k < 1 makes no sense.
        regions.clear();
        regions.reserve(k);
        auto intervals_copy = intervals;
        auto largest_interval = std::max_element(intervals_copy.begin(), intervals_copy.end(), [](const Interval& a, const Interval& b) {
  ...
  /// CoronaChecker
    bool CoronaChecker::SatisfiesConstraints() const {
      ...
      bool CoronaChecker::Inspect(const RadiiRegion &radii_) {
        Reset();
        radii = radii_.GetIntervals();
        frequency_table.resize(radii.size());
      ...
      BasicChecker::BasicChecker( const BaseType &packing_radius_, 
                                  ...
                                  const size_t &size_upper_bound_): packing_radius{packing_radius_},
      ...
      bool BasicChecker::Inspect(const RadiiRegion& radii_) {
        auto corona_checker = CoronaChecker{precision_upper_bound};
        if (!corona_checker.Inspect(radii_)) {
          return false;
        }
        auto basic_generator = BasicGenerator{radii_.GetIntervals(), packing_radius, precision_upper_bound, size_upper_bound};
        auto status = basic_generator.Generate();
        return status != PackingStatus::invalid;
      }
